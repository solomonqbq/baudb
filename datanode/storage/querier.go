...
func NewMergedQuerier(queriers []Querier) *MergedQuerier {
	return &MergedQuerier{
		blocks: queriers,
	}
}

func (q *MergedQuerier) LabelValues(n string) ([]string, error) {
	return q.lvals(q.blocks, n)
}

// LabelNames returns all the unique label names present querier blocks.
func (q *MergedQuerier) LabelNames() ([]string, error) {
...
func (q *MergedQuerier) lvals(qs []Querier, n string) ([]string, error) {
	if len(qs) == 0 {
		return nil, nil
	}
	if len(qs) == 1 {
...
func (q *MergedQuerier) LabelValuesFor(string, labels.Label) ([]string, error) {
	return nil, fmt.Errorf("not implemented")
}

func (q *MergedQuerier) Select(ms ...labels.Matcher) (SeriesSet, error) {
	return q.sel(q.blocks, ms)
}

func (q *MergedQuerier) sel(qs []Querier, ms []labels.Matcher) (SeriesSet, error) {
	if len(qs) == 0 {
	...
	var (
		errors     = make([]error, len(qs))
		seriesSets = make([]SeriesSet, len(qs))
		wg         sync.WaitGroup
	)
	...
	for i, q := range qs {
		...
		go func(idx int, q Querier) {
			...
			seriesSets[idx], errors[idx] = q.Select(ms...)
		}(i, q)
	}
	wg.Wait()
	...
	set := newMergedSeriesSet(seriesSets[0], seriesSets[1])
	for i := 2; i < len(seriesSets); i++ {
		set = newMergedSeriesSet(set, seriesSets[i])
	}
	return set, nil
}
...
type MergedVerticalQuerier struct {
	MergedQuerier
}

func (q *MergedVerticalQuerier) Select(ms ...labels.Matcher) (SeriesSet, error) {
	return q.sel(q.blocks, ms)
}

func (q *MergedVerticalQuerier) sel(qs []Querier, ms []labels.Matcher) (SeriesSet, error) {
	if len(qs) == 0 {
		return EmptySeriesSet(), nil
	}
	if len(qs) == 1 {
	...
	set := newMergedVerticalSeriesSet(seriesSets[0], seriesSets[1])
	for i := 2; i < len(seriesSets); i++ {
		set = newMergedVerticalSeriesSet(set, seriesSets[i])
	}
	return set, nil
}
...
func (s *chunkSeries) Iterator() SeriesIterator {
	...
	//	} else {
	//		_, isHeadChunk := c.Chunk.(*safeChunk)
	//		if !isHeadChunk {
	//			atomic.AddUint32(&chunksNumRead, 1)
	//			//atomic.AddUint64(&chunksBytesRead, uint64(len(c.Chunk.Bytes())))
	//		}
	//	}
	//}
	//
	...
	return newChunkSeriesIterator(s.cr, s.chunks, s.intervals, s.mint, s.maxt)
}
...
func newChunkSeriesIterator(cr ChunkReader, cs []chunks.Meta, dranges Intervals, mint, maxt int64) *chunkSeriesIterator {
	csi := &chunkSeriesIterator{
		...
		cr: cr,
		ch: make(chan int, len(cs)),
	}
	csi.resetCurIterator(false)
	return csi
}

func (it *chunkSeriesIterator) populateChunks() {
	i := it.i
	go func() {
		for i < len(it.chunks) {
			c := &it.chunks[i]
			if c.Chunk == nil {
				if chk, err := it.cr.Chunk(c.Ref); err == nil {
					c.Chunk = chk
				}
			}
			it.ch <- i
...
func (it *chunkSeriesIterator) resetCurIterator(waitPopulate bool) {
	c := &it.chunks[it.i]
	...
	if c.Chunk == nil {
		chk, err := it.cr.Chunk(c.Ref)
		if err != nil {
			it.i++
			it.resetCurIterator(waitPopulate)
			return
		}
		c.Chunk = chk
	}
	...
	if len(it.intervals) == 0 {
		it.cur = c.Chunk.Iterator(it.cur)
		return
	}
	...
	it.bufDelIter.it = c.Chunk.Iterator(it.bufDelIter.it)
...
func (noopSeriesIterator) Seek(int64) bool {
	return false
}
