...
func (c *ShardClient) Add(ctx context.Context, req *backendmsg.AddRequest) error {
	...
	resp, err := visitor.VisitAll(shard, func(node *meta.Node) (msg.Message, error) {
		if c.localStorage != nil && node.IP == vars.LocalIP && node.Port == vars.Cfg.TcpPort {
			return c.localStorage.HandleAddReq(req), nil
		}
...
func (c *ShardClient) Close() error {
	...
	nodes := meta.GetNodes(c.shardID)
	if len(nodes) > 0 {
		for _, node := range nodes {
			err := defaultFactory.destroy(node.Addr())
			if err != nil {
