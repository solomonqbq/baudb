...
func (api *API) SelectQuery(request *gatewaymsg.SelectRequest) *gatewaymsg.SelectResponse {
	result, err := api.selectQuery(request.Start, request.End, request.Timeout, request.Query, request.Offset, request.Limit)
	if err != nil {
...
func (api *API) SeriesLabels(request *gatewaymsg.SeriesLabelsRequest) *gatewaymsg.SeriesLabelsResponse {
	labels, err := api.seriesLabels(request.Matches, request.Start, request.End, request.Timeout)
	if err != nil {
...
func (api *API) LabelValues(request *gatewaymsg.LabelValuesRequest) *msg.LabelValuesResponse {
	values, err := api.labelValues(request.Name, request.Constraint, request.Timeout)
	if err != nil {
...
func (api *API) Ingest(request *gatewaymsg.AddRequest) error {
	var err error
	var appender backend.Appender

	if v := api.appenderPool.Get(); v != nil {
		appender = v.(backend.Appender)
	} else {
		appender, err = api.Backend.Appender()
		if err != nil {
...
func (api *API) HttpSelectQuery(c *fasthttp.RequestCtx) {
	exeHttpQuery(c, func() (interface{}, error) {
		...
		return api.selectQuery(start, end, timeout, query, offset, limit)
	})
}

//list labels of matched series
func (api *API) HttpSeriesLabels(c *fasthttp.RequestCtx) {
	exeHttpQuery(c, func() (interface{}, error) {
		var httpArgs *fasthttp.Args
		...
		return api.seriesLabels(matches, start, end, timeout)
	})
}

//list values of the matched label
func (api *API) HttpLabelValues(c *fasthttp.RequestCtx) {
	exeHttpQuery(c, func() (interface{}, error) {
		...
		return api.labelValues(name, constraint, timeout)
	})
}

func (api *API) HttpIngest(jobBase64Encoded bool) func(c *fasthttp.RequestCtx) {
	return func(c *fasthttp.RequestCtx) {
		c.Error("not implements", 404)
	}
}

func (api *API) selectQuery(startT, endT, timeout, query string, offset int, limit int) (*queryResult, error) {
	...
	qry := api.QueryEngine.NewQuery(api.Backend, query, start, end, offset, limit)
...
func (api *API) seriesLabels(matches []string, startT, endT, timeout string) ([][]msg.Label, error) {
	...
	q, err := api.Backend.Querier(ctx, ts.FromTime(start), ts.FromTime(end))
	if err != nil {
		return nil, err
	}
	defer q.Close()
...
func (api *API) labelValues(name, constraint, timeout string) ([]string, error) {
	...
	q, err := api.Backend.Querier(ctx, mint, maxt)
	if err != nil {
		return nil, err
	}
	defer q.Close()
